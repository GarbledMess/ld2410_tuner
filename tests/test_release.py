    ...
    def test_versions_only_release_on_increase(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            with patch.object(release, "git", return_value=json.dumps({"version": "2.0.0"})):
                with self.assertRaises(ValueError):
                    release.changed_version(root, "a" * 40)
        ...
        class API:
            def request(self, path, method="GET", payload=None, **kwargs):
                ...
                if path == "/releases" and method == "POST":
                    return {"id": 7, "draft": True, "assets": []}
                return None

        with tempfile.TemporaryDirectory() as directory:
            archive = Path(directory) / release.ASSET
                        ...
                        "assets": [
                            {
                                ...
                                "digest": "wrong"
                                if wrong_digest
                                else "sha256:" + hashlib.sha256(b"archive").hexdigest(),
                            }
                        ],
                    }
