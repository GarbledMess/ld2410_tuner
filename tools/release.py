...
def build(root, output):
    ...
    for name in RUNTIME_FILES:
        ...
        if path.is_symlink() or any(
            (package / parent).is_symlink() for parent in Path(name).parents
        ):
...
def publish(api, version, sha, archive):
    ...
    _upload_asset(api, release, content, digest)
    _publish_draft(api, release, version)
...
def main():
    ...
    if args.publish:
        publish(
            GitHub(os.environ["GITHUB_REPOSITORY"], os.environ["GH_TOKEN"]), version, sha, archive
        )


def _verify_tag(api, tag, sha):
...
def _upload_asset(api, release, content, digest):
    asset = next((a for a in release.get("assets", []) if a["name"] == ASSET), None)
    if asset:
        if asset.get("digest") != digest:
            raise ValueError(
                "Existing release asset differs or cannot be verified; refusing to replace it"
            )
    else:
...
def _publish_draft(api, release, version):
    if release["draft"]:
        latest = api.request("/releases/latest", missing_ok=True)
        make_latest = latest is None or version_tuple(version) > version_tuple(
            latest["tag_name"].removeprefix("v")
        )
        api.request(
            f"/releases/{release['id']}",
            "PATCH",
            {"draft": False, "make_latest": str(make_latest).lower()},
        )
